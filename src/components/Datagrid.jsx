...
const Datagrid = ({ actuli, catId }) => {
  ...
  const expandir = (index) => {
    console.log(index)
    mostrar === index ? setMostrar(-1) : setMostrar(index)
  }
  ...
  return (
    ...
    <div className='px-5 flex flex-col justify-center'>
      ...
      {datosgasto.map(({ detDesc, detFecha, detMonto }, index) => (
        <div key={index} className='rounded-xl border my-2 px-3 shadow'>
          <div onClick={() => expandir(index)} className="grid grid-cols-3 text-lg pt-1.5">
            <div className='grid grid-rows-2 grid-cols-3 justify-start col-span-2  lg:flex-row lg:justify-center'>
              <p className='row-start-2 row-end-3 col-span-3 font-normal text-xs -mt-1 text-gray-500 lg:text-lg lg:mt-0 lg:text-black lg:font-normal lg:row-span-2 lg:col-span-1 '>{shortDate(detFecha)}</p>
          ...
          <div id={index} className={mostrar === index ? 'block text-gray-500 border-t-2 py-2' : 'hidden'} >
            <p>{detDesc}</p>
            <div className='flex justify-between'>
            <p>{longDate(detFecha)}</p>
            <p>{detMonto}</p>
            </div>
          </div>
