...
import FormTrac from '../cuenta/Transacc/FormTran'

const Nav = ({ children }) => {
  const { ctxCuenta, ctxCamCue, ctxCamTos } = contexto()

  const [usFormTran, setUsFormTran] = useState(false)

  const clForTran = () => setUsFormTran(false)

  const { push } = useRouter()

  // Consumiendo el hook de API
  const datainfo = apiSure('cuenta/iduno')
    ...
    if (!ctxCuenta) {
      if (datainfo) {
        if (datainfo.length === 0) {
          ctxCamCue(datainfo)
        } else ctxCamCue(datainfo[0].cueId)
      }
    }
  }, [datainfo])
  ...
  if (!ctxCuenta) return Toast(true, 1)

  return (
    <>
    <FormTrac abrir={usFormTran} cerrar={clForTran}/>
<div className='container mx-auto '>
  <div className='pb-5 '>
    {children}
  </div>
  <div className="container mx-auto z-40 text-xs fixed grid grid-cols-5 w-full border-t max-[1023px]:bottom-0 bg-white">
    <Link href={'/cuenta/config'} className="py-1 col-span-1 flex flex-col items-center justify-end  fill-black">
    ...
    <button onClick={newinfo} href={'#new'} className="py-1 col-span-1 flex flex-col items-center justify-end fill-black">
